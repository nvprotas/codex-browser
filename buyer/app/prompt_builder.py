...
6. Для Litres SberPay находится за способом оплаты "Российская карта": выбери "Российская карта", нажми "Продолжить", дождись iframe payment с адресом вида `https://payecom.ru/pay_ru?orderId=...` и верни orderId из параметра `orderId` в src этого iframe.
7. Если сценарий завершен успешно, верни status=completed; в message краткий итог, в order_id передай orderId, найденный только на странице SberPay или в платежном iframe SberPay. Если orderId найден не на странице SberPay/не в платежном iframe SberPay или выбран только СБП/SBP/FPS, верни order_id=null.
8. При status=completed для Litres верни payment_evidence={{"source":"litres_payecom_iframe","url":"<iframe src>"}}; url должен быть тем самым `https://payecom.ru/pay_ru?...orderId=...`, из которого взят order_id.
9. Если не хватает данных, верни status=needs_user_input и задай один конкретный вопрос в поле message.
10. Если сценарий невозможно продолжать, верни status=failed и объяснение в message.
11. Если в последнем ответе пользователя появились новые долговременные факты о пользователе, верни их в profile_updates как массив коротких строк.
12. В profile_updates добавляй только новые факты, которых еще нет в постоянном профиле пользователя. Если новых фактов нет, верни пустой массив.
13. В profile_updates нельзя включать auth, storageState, cookies, платежные данные и одноразовые детали текущего заказа.
