...
8. Если не хватает данных, верни status=needs_user_input и задай один конкретный вопрос в поле message.
9. Если сценарий невозможно продолжать, верни status=failed и объяснение в message.
10. Если в последнем ответе пользователя появились новые долговременные факты о пользователе, верни их в profile_updates как массив коротких строк.
11. В profile_updates добавляй только новые факты, которых еще нет в постоянном профиле пользователя. Если новых фактов нет, верни пустой массив.
12. В profile_updates нельзя включать auth, storageState, cookies, платежные данные и одноразовые детали текущего заказа.
