        ...
        async with self._lock:
            ...
            for session_id, events in self._events_by_session.items():
                ...
                for event in events:
                    ...
                    if event.event_type == 'payment_unverified':
                        raw_order_id = event.payload.get('order_id')
                        raw_order_id_host = event.payload.get('order_id_host')
                        ...
                        payment_provider = raw_provider if isinstance(raw_provider, str) and raw_provider else None
                        status = 'unverified'
                        waiting_reply_id = None
                        ask_question = None
                        ask_options = []
                    ...
                    if event.event_type == 'scenario_finished':
                        scenario_status = event.payload.get('status')
                        if not (status == 'unverified' and scenario_status == 'completed'):
