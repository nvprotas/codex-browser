...
| `docker-compose.yml` | Локальный стек `postgres` + `browser` + `buyer` + `micro-ui` + `eval_service`. | `.env`, env `EVAL_CALLBACK_SECRET`, `EVAL_AUTH_SOURCE`, `TRUSTED_CALLBACK_URLS`, `SBER_AUTH_SOURCE`, `SBER_COOKIES_API_URL`, `SBER_COOKIES_API_SCOPE`, `SBER_COOKIES_API_TIMEOUT_SEC`, `SBER_COOKIES_API_RETRIES`, `MICRO_UI_ENABLE_EVAL_STUBS`, bind mounts `CODEX_AUTH_JSON_PATH`, `USER_BUYER_INFO_PATH`, `EVAL_AUTH_PROFILES_HOST_DIR`. | Host-порты только на loopback: `127.0.0.1:5432`, `127.0.0.1:6901`, `127.0.0.1:8000`, `127.0.0.1:8080`, `127.0.0.1:8090`; CDP `9223` доступен только внутри docker-сети как `http://browser:9223`; volume `buyer-postgres-data`; `CODEX_AUTH_JSON_PATH` монтируется в `buyer` и `eval_service` как `/run/codex/host-auth`; `buyer` может получать SberId cookies из внешнего сервиса при `SBER_AUTH_SOURCE=external_cookies_api` и передавать `SBER_COOKIES_API_SCOPE` как `X-Ratatouille-Sber-Scope`; `eval_service` при `EVAL_AUTH_SOURCE=buyer_runtime` не передает inline `storageState` и делегирует auth runtime `buyer`; eval auth-профили читаются из host-директории и монтируются в `/run/eval/auth-profiles`; eval stubs в debug `micro-ui` включаются только при `MICRO_UI_ENABLE_EVAL_STUBS=true`. | Неверные env/mounts ломают авторизацию Codex, профиль пользователя, external cookies source или eval callbacks; отсутствующая host-директория или файл `<auth_profile>.json` приводит eval-case к `skipped_auth_missing` при `EVAL_AUTH_SOURCE=auth_profiles`; недоступный `browser` блокирует агентный шаг; удаленный доступ к loopback-портам требует VPN/SSH tunnel/authenticated reverse proxy. |
| `docker-compose.openclaw.yml` | Standalone compose для развертывания рядом с `openclaw`: только `postgres`, `browser`, `buyer`, без `eval_service` и debug-модуля `micro-ui`. | `.env`, обязательные `MIDDLE_CALLBACK_URL` и `SBER_COOKIES_API_URL`, optional `SBER_COOKIES_API_SCOPE`, bind mounts `CODEX_AUTH_JSON_PATH`, `USER_BUYER_INFO_PATH`. | `buyer` публикуется на `${BUYER_BIND_ADDR:-127.0.0.1}:${BUYER_PORT:-8000}`, noVNC на `${NOVNC_BIND_ADDR:-127.0.0.1}:${NOVNC_PORT:-6901}`, Postgres на `${POSTGRES_BIND_ADDR:-127.0.0.1}:${POSTGRES_PORT:-5432}`; callbacks отправляются во внешний `middle`; SberId cookies берутся из external cookies API по умолчанию; `SBER_COOKIES_API_SCOPE` передается как `X-Ratatouille-Sber-Scope`; `buyer` получает `host.docker.internal` для доступа к сервисам host-машины. | Неверный `MIDDLE_CALLBACK_URL` ломает доставку событий в middle; неверный `SBER_COOKIES_API_URL` или scope переводит auth в guest-flow; открытые bind addr требуют доверенного периметра. |
...
| `scripts/evolve_buyer_loop.py` | Standalone Python stdlib CLI для MVP-A evolve-loop `buyer`. | CLI commands `doctor`, `run`, `continue`, `compare`; `--repo`, `--reports-dir`, `--case-id`, baseline/candidate eval URLs, `--patch-mode placeholder|external-command`, optional `--patch-command`, `--candidate-prepare-command`, `--allowed-path`, explicit `--skip-candidate-eval` для baseline-only smoke. | Читает existing `eval_service` HTTP API; создает candidate branch/worktree `refs/heads/evolve/cand-*`; пишет `.tmp/evolve/**` artifacts, `summary.md`, `latest.json`, `delta_report.json`, redacted `candidate.diff`, `patch-request.json`, `patch-manifest.json`; может сделать один candidate commit сам или принять candidate commit, уже созданный external patch command. | Недоступный eval endpoint, одинаковые baseline/candidate URLs, mismatch case fingerprints, judge failure, `waiting_user`/handoff, git/patch failure, forbidden unstaged/staged/committed path diff/path traversal, external patch command сдвинул `HEAD`, но оставил незакоммиченный diff, artifact/redaction failure; MVP-A не push-ит ветки и не двигает champion. |
| `tools/propose_buyer_patch.py` | External patch command для `scripts/evolve_buyer_loop.py`, который запускает вложенный `codex exec` в candidate worktree. | Env `EVOLVE_CANDIDATE_WORKTREE`, `EVOLVE_PATCH_REQUEST`, `EVOLVE_PATCH_MANIFEST`, `EVOLVE_ALLOWED_PATHS`, optional `EVOLVE_CODEX_BIN`, `EVOLVE_CODEX_MODEL`, `EVOLVE_CODEX_REASONING_EFFORT`, `EVOLVE_CODEX_SANDBOX_MODE`, `EVOLVE_CODEX_TIMEOUT_SEC`. | Передает Codex eval context, judge recommendations и allowed paths; задает цель снижать число agent/browser шагов и токенов без ухудшения quality/safety/payment/evidence; не передает `EVOLVE_PATCH_MANIFEST` во вложенный процесс; пишет рядом с manifest `codex-exec.prompt.txt`, `codex-exec.stdout.jsonl`, `codex-exec.stderr.log`; ожидает, что вложенный Codex оставит рабочий diff без записи в `.git`; затем сам строит `patch-manifest.json` из worktree diff. Если вложенный Codex все-таки сделал commit, manifest строится из commit subject и `git diff-tree`. | Невалидный env, недоступный `codex`, timeout, отсутствие candidate changes, пустой committed/worktree diff или невалидный generated manifest завершают команду ненулевым кодом с путями к логам; финальную path/secret validation и commit все равно выполняет evolve-loop. |
...
| `docs/buyer-agent/cdp-tool.md` | Runtime manual для вызова `buyer/tools/cdp_tool.py`. | CDP доступен buyer-agent через shell command execution, а не отдельный нативный tool-вызов Codex; доступные команды и их параметры, milestone/evidence проверки после state-changing действий, recovery и правила экономии HTML. |
...
| `docs/buyer-agent/instructions/*.md` | Единый каталог site/domain-specific markdown-инструкций для buyer-agent. | Агент смотрит список файлов в каталоге, выбирает релевантные инструкции по текущему сайту или задаче; Litres PayEcom evidence и checkout fast path с точными selector-ами, Brandshop search/cart/checkout/YooMoney evidence; fixtures не должны становиться hardcoded SKU. |
| `docs/buyer-agent/instructions/litres.md` | Site-specific fast path для Litres generic-agent. | Если агент уже на `/purchase/ppd/` с `method=russian_card&system=sbercard`, а URL checkout указывает на целевую книгу, он пропускает повторный поиск, общие snapshots/links, проверяет наличие PayEcom iframe или кнопки `Продолжить`, при timeout-ошибке сразу жмет `Попробовать снова`; `snapshot [data-testid="ppd-checkout"]` остается проверкой только когда URL не подтверждает товар или способ оплаты еще не выбран. |
...
- git repo и `--base-ref`;
- optional external patch command через `EVOLVE_*` env и `patch-request.json`; command может либо оставить рабочий diff для commit-а самим loop, либо самостоятельно сделать candidate commit;
...
- committed files from external patch commands валидируются вместе со staged/unstaged/untracked/deleted files и `patch-manifest.touched_paths`; если external command уже сделал commit, loop сохраняет committed diff и отклоняет оставшийся незакоммиченный diff;
...
- external Sber cookies API: `SBER_AUTH_SOURCE`, `SBER_COOKIES_API_URL`, `SBER_COOKIES_API_SCOPE`, `SBER_COOKIES_API_TIMEOUT_SEC`, `SBER_COOKIES_API_RETRIES`;
...
- полный cookies endpoint, optional scope, timeout и retry budget из настроек;
- `GET` по полному URL из `SBER_COOKIES_API_URL`, с заголовком `X-Ratatouille-Sber-Scope` при заданном `SBER_COOKIES_API_SCOPE`;
...
Runtime auth-профили берутся из host-директории `EVAL_AUTH_PROFILES_HOST_DIR`, которая bind-mounted в контейнер как `/run/eval/auth-profiles` и внутри сервиса читается через `EVAL_AUTH_PROFILES_DIR`. Для `auth_profile: litres_sberid` ожидается файл `litres_sberid.json`; этот же профиль используется Brandshop smoke-case, потому что SberId auth общий. Эти файлы являются секретами, не входят в image и не должны храниться в repo. При `EVAL_AUTH_SOURCE=buyer_runtime` `eval_service` не загружает эти профили и отправляет задачу без inline `storageState`, чтобы `buyer` получил SberId cookies через собственный runtime-источник, например внешний cookies API.
...
- repository smoke-case templates явно стартуют с главных страниц сайтов: Litres с `https://www.litres.ru/`, Brandshop с `https://brandshop.ru/`; поиск товара остается задачей buyer-agent, а product identity и размер проверяются как constraints.
...
| Evolve loop artifacts | `.tmp/evolve/latest.json`, `.tmp/evolve/cycles/<cycle_id>/**` | `cycle.json`, `summary.md`, `operator-action.json`, baseline/candidate eval request/result JSON, `patch-request.json`, `patch-manifest.json`, `patch-diffstat.json`, `candidate.diff`, `candidate-prepare.log`, `delta_report.json`, `codex-exec.prompt.txt`, `codex-exec.stdout.jsonl`, `codex-exec.stderr.log`, redacted logs. | Raw HTTP headers/bodies, tokens/cookies/authorization/password/secret/storageState, raw `orderId`, payment URLs. |
...
| External Sber cookies API | `ExternalSberCookiesClient`, `BuyerService._resolve_session_auth` | Машинная загрузка cookies через `GET` по полному URL из `SBER_COOKIES_API_URL` при `SBER_AUTH_SOURCE=external_cookies_api` и отсутствии inline auth; optional `SBER_COOKIES_API_SCOPE` передается как `X-Ratatouille-Sber-Scope`. | Timeout, HTTP/network error, invalid JSON, invalid/empty cookies payload, неверный scope; все переходят в `auth_external_*` reason-code и guest-flow. |
...
| Git CLI | `scripts/evolve_buyer_loop.py` | Проверка repo/base ref, создание candidate branch/worktree, diff validation, allowed-path add и candidate commit, либо чтение committed diff от external patch command. | Git binary/repo/base ref недоступны, invalid branch name, forbidden touched path, `git diff --check` failure, commit failure. |
| External patch command | `scripts/evolve_buyer_loop.py` при `--patch-mode external-command` | Применяет code/prompt/playbook изменения в candidate worktree, пишет `patch-manifest.json`; может самостоятельно сделать candidate commit. | Non-zero exit, missing binary, timeout, missing/invalid manifest, empty diff, forbidden unstaged/staged/committed path diff, already-committed patch с оставшимся незакоммиченным diff. |
| `codex exec` | `tools/propose_buyer_patch.py` | Анализирует `patch-request.json`, eval artifacts и код в candidate worktree, затем оставляет рабочий diff без `git add`/`git commit`; `propose_buyer_patch.py` сам пишет manifest в evolve artifacts, а stdout/stderr и prompt сохраняет для диагностики даже при timeout или отсутствии изменений. | Missing credentials, timeout, non-zero exit, отсутствие изменений, попытка работать с `.git` в sandbox worktree. |
...
| `eval_service/tests/test_case_registry.py` | Загрузка YAML eval templates, repository smoke cases, disabled templates и ошибки template variables. |
