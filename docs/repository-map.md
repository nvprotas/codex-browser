...
| `docs/buyer-agent/instructions/litres.md` | Site-specific fast path для Litres generic-agent. | Если агент уже на `/purchase/ppd/` с `method=russian_card&system=sbercard`, а URL checkout указывает на целевую книгу, он пропускает повторный поиск, общие snapshots/links и сразу нажимает `Продолжить` с ожиданием PayEcom iframe; `snapshot [data-testid="ppd-checkout"]` остается проверкой только когда URL не подтверждает товар или способ оплаты еще не выбран. |
