...
| `docs/buyer-agent/cdp-tool.md` | Runtime manual для вызова `buyer/tools/cdp_tool.py`. | CDP доступен buyer-agent через shell command execution, а не отдельный нативный tool-вызов Codex; доступные команды, milestone/evidence проверки после state-changing действий, recovery и правила экономии HTML. |
