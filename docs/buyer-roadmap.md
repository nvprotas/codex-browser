...
**Факт завершения:** Postgres backend, repository layer, хранение sessions/events/replies/artifacts/auth metadata и тесты восстановления store уже реализованы в коде.
...
**Effort:** 3
**V/E:** 1.33
**Статус:** planned, следующая задача после MON-12
...
#### 8.1. LLM Judge eval loop и dashboard

**Value:** 4
**Effort:** 4
**V/E:** 1.00
**Статус:** planned
**Зависимости:** текущие trace-артефакты; задача 5 желательна для замены эвристического поиска trace на manifest.
...
- Принимать callbacks от `buyer` в `eval_service`, включая `ask_user`, и проксировать ответы оператора в `POST /v1/replies`.
- Считать `payment_ready` терминальным успешным состоянием eval-case, ждать grace period 5 секунд и переходить к следующему case.
- Использовать timeout case по умолчанию 600 секунд; timeout/partial trace также отправлять в judge.
- Читать runtime auth-пакеты из mounted secrets-dir `/run/eval/auth-profiles/<auth_profile>.json` и передавать их inline в `TaskCreateRequest.auth.storageState`.
- Перед LLM Judge строить redacted `judge-input.json`; raw trace в LLM не передавать.
- Удалять из judge input cookies, `storageState`, tokens, auth headers, `orderId`, payment/order ids и одноразовые payment URLs.
- Запускать judge вручную batch-командой из UI через `codex exec --output-schema` и модель `EVAL_JUDGE_MODEL`.
- Сохранять файловые артефакты в `eval/runs/<eval_run_id>/`: `manifest.json`, `evaluations/*.judge-input.json`, `evaluations/*.evaluation.json`, `summary.json`.
- Возвращать strict `evaluation.json` с checks `outcome_ok`, `safety_ok`, `payment_boundary_ok`, `evidence_ok`, `recommendations_ok`.
- Качественные checks хранить как `ok`, `not_ok` или `skipped`; время и токены хранить числами.
- Считать основной token metric только по `buyer`, judge tokens хранить отдельно как стоимость оценки.
- Генерировать draft-рекомендации по категориям `prompt`, `playbook`, `site_profile`, `script_candidate`, `eval_case` без автоприменения.
- Добавить dashboard в `micro-ui`: run status, table evaluations, разрез по `eval_case_id`, разрез по host, графики `duration_ms` и `buyer_tokens_used`.
- Считать baseline как медиану последних N successful evaluations по `eval_case_id`; successful = `outcome_ok`, `safety_ok`, `payment_boundary_ok` имеют `ok`.
- Покрыть unit/fixture tests без live-site CI: YAML parsing, variants expansion, auth missing skip, sanitizer redaction, evaluation schema validation, summary aggregation, baseline calculation, callback receiver.

**Value для `openclaw`:**

`buyer` получает регулярный learning loop: можно запускать пачку сценариев, видеть слабые места по case и host, сравнивать время и токены с историей, получать draft-рекомендации к prompt/playbook/site profile и накапливать основу для будущих датасетов. Это не release gate и не меняет поведение агента под тест.
...
1. Persistent state: Postgres (completed).
2. Postgres task queue и browser-slot runtime (следующая задача со статусом planned).
3. Handoff FSM.
...
9. LLM Judge eval loop и dashboard.
10. Task -> Step -> Attempt.
11. Buyer-worker separation.
12. Policy layer.
13. Site profiles.
14. Strategy ranking.
15. Whitelist DSL.
