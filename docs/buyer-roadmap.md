...
- Добавить Redis в compose и настройки `buyer`.
- Ввести lock на активное выполнение сессии: один runner на одну сессию.
- Ввести TTL runtime-маркеры для handoff, active browser context и callback delivery attempts.
- Заменить глобальный `MAX_ACTIVE_SESSIONS=1` на конфигурируемые лимиты:
  - `max_active_jobs_per_worker`;
  - `max_handoff_sessions`;
  - доменные лимиты для одного магазина.
- Добавить защиту от двойного старта одной сессии при повторном запросе или рестарте worker.
...
**Effort:** 4  
**V/E:** 1.00  
**Статус:** planned после persistence и locks  
**Зависимости:** задачи 1-2.
...
- Worker должен poll-ить или забирать задачи через очередь/lock.
...
- Подготовить возможность нескольких worker-ов без двойного выполнения одной сессии.
