...
- Конфигурация внешнего источника:
  ...
  - `SBER_COOKIES_API_SCOPE` как значение заголовка `X-Ratatouille-Sber-Scope`, если внешний сервис требует scope;
...
- Внешний cookies API читается через `GET` по полному URL из `SBER_COOKIES_API_URL`; при заданном `SBER_COOKIES_API_SCOPE` запрос включает заголовок `X-Ratatouille-Sber-Scope`. Write-path остается во внешнем auth-контуре.
...
- После выполнения `buyer` может предлагать новый скрипт:
  ...
  - сразу создает отдельную git-ветку с concrete script diff и verifier, если candidate patchable,
  - прикладывает краткую рекомендацию по применению,
  - не включает merge/публикацию/автоприменение по умолчанию.
...
- Для ускорения evolution loop patchable draft-кандидат сразу материализуется в отдельную git-ветку с одним основным commit. Такая ветка не считается активацией: следующий runtime-прогон использует изменение только после merge и явной activation.
