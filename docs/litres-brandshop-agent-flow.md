...
| `docs/buyer-agent/instructions/brandshop.md` | Brandshop search/cart/checkout instruction и YooMoney evidence requirements; Air Jordan Retro High OG 45 EU остается примером/fixture, а не hardcoded SKU. |
...
| Eval | `eval/cases/brandshop_purchase_smoke.yaml` включен под Air Jordan Retro High OG 45 EU. |
...
3. Ввести product identity в catalog search input с placeholder `Искать в каталоге`; для целевого instruction это `Air Jordan Retro High OG`.
...
6. Выбрать вариант товара, соответствующий product identity и размеру; если page data не позволяет надежно подтвердить соответствие, вернуть `needs_user_input`.
7. Открыть product page и перед добавлением проверить product identity и размер.
...
- старт с главной страницы `https://www.litres.ru/`;
...
- auth profile задан как `litres_sberid`, общий с Litres SberId eval-профилем;
- старт с главной страницы `https://brandshop.ru/`;
- smoke-задача ориентирована на generic instruction: Air Jordan Retro High OG размера `45 EU`;
...
| Eval Brandshop | Активен | Требует доступного `litres_sberid` auth profile; без него eval-run будет пропущен как auth-missing. |
...
| `eval/cases/brandshop_purchase_smoke.yaml` | Brandshop smoke-case для Air Jordan Retro High OG 45 EU и текущее значение `enabled`. |
