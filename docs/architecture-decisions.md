...
- Конфигурация external source: `SBER_AUTH_SOURCE=inline_only|external_cookies_api`, полный endpoint `SBER_COOKIES_API_URL`, optional scope `SBER_COOKIES_API_SCOPE`, `SBER_COOKIES_API_TIMEOUT_SEC`, `SBER_COOKIES_API_RETRIES`; default `inline_only`.
...
- Внешний cookies API читается через `GET` по полному URL из `SBER_COOKIES_API_URL`; если задан `SBER_COOKIES_API_SCOPE`, `buyer` передает его в заголовке `X-Ratatouille-Sber-Scope`. API возвращает cookies, которые `buyer` валидирует и преобразует в Playwright `storageState` с пустым `origins`; write-path остается вне ответственности `buyer`.
