...
- Baseline-only smoke is explicit: no candidate eval endpoint requires `--skip-candidate-eval` and then writes `verdict.status="inconclusive"`, reason `candidate_eval_endpoint_absent`.
...
13. Validate diff: there must be a diff; touched paths, untracked files and pre-staged files must match allowed paths and forbidden-path policy; `git diff --check` passes.
14. Commit one candidate change.
15. Write `patch-manifest.json`, `patch-diffstat.json`, `candidate.diff`.
16. If `--candidate-eval-base-url` is absent, require explicit `--skip-candidate-eval`, write inconclusive report and stop.
...
For MVP-A speed, redaction is intentionally small but must cover all text artifacts:
...
- Redact Bearer/JWT-looking values, Authorization/Cookie/Set-Cookie headers, token/password/secret/orderId key-value forms and payment URLs.
...
- `run` with only baseline endpoint plus explicit `--skip-candidate-eval` writes inconclusive report and exits `0`.
